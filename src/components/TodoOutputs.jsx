...
const TodoOutputs = () => {
  ...
  const handleSaveEdit = (id) => {
    axios
      .patch(`${apiUrl}?id=eq.${id}`, { Car: editText }, { headers })
      .then(() => {
        setData((prevData) =>
          prevData.map((item) =>
            item.id === id ? { ...item, Car: editText } : item
          )
        );
        setEditId(null);
        setEditText("");
            ...
            {data.map((el, index) => (
              <li key={el.id} className="todo">
                {editId === el.id ? (
                  ...
                  <>
                    <span className="todo-number">{index + 1}.</span> {el.Car}
                    <div className="right">
                      <div className="icons">
                        <div className="icon-container">
                          <FaRegEdit
                            className="icon"
                            onClick={() => handleEdit(el.id, el.Car)}
                          />
                        </div>
                        <div className="icon-container">
                          <TiDeleteOutline
